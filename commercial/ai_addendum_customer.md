# AI / Gen-AI Addendum - Customer side

> You're buying AI / gen-AI features on top of a SaaS or services deal. Secures output ownership, a strict no-training default, an output-infringement indemnity, and a real service warranty despite the hallucination disclaimer.

**Side:** Customer · **Jurisdiction:** US

**Best fit for:**

- Adopting a vendor's AI / gen-AI features as the customer
- Adding an AI addendum to a SaaS or services agreement

**Tags:** `ai`, `genai`, `llm`, `addendum`, `customer`
...
AI Outputs may be inaccurate; the customer applies human review for material decisions, but the vendor warrants the service performs its advertised functions and does not disclaim ALL accuracy where it markets specific AI capabilities. Data-breach and IP-output claims sit under the super-cap / uncapped tier, not a nominal AI-specific cap.
...
- Blanket 'no warranty of accuracy' where the vendor markets specific AI capabilities
- AI failures capped at a nominal amount below the general cap
- Hallucination disclaimer used to escape the service warranty entirely

**Rationale**

A blanket accuracy disclaimer plus a nominal AI-specific cap would let the vendor market specific AI capabilities while owing nothing when they fail, so the service warranty and the super-cap tier need to survive the hallucination disclaimer.
...
Vendor indemnifies for third-party claims that the customer's use of Outputs generated by the vendor's (unmodified) models infringes a copyright or other IP right, with carve-outs only for customer-supplied prompts / data and customer modifications. NO training-data carve-out -- the vendor, not the customer, chose the training data (mirrors the Microsoft Copilot / OpenAI / Anthropic customer-copyright commitments).
...
1. Vendor indemnifies for third-party IP-infringement claims on unmodified-model Outputs with no training-data carve-out, carving out only customer prompts, data, and modifications.
2. Output indemnity applies but is capped at the general liability cap rather than uncapped, still with no training-data carve-out.
3. Output indemnity applies subject to the customer using the current model version and vendor-provided content filters.

**Walk-away floor**

No AI-output infringement indemnity, a broad training-data carve-out, or an output indemnity capped at a nominal amount.

**Escalation triggers**

- No AI-output infringement indemnity
- Broad training-data carve-out shifting model-training risk to the customer
- Indemnity excludes model Outputs (covers only the software)
- Output indemnity capped at a nominal amount

**Rationale**

Because purely AI-generated Outputs may not be copyrightable, the output-infringement indemnity is our only real backstop, so a training-data carve-out or a nominal cap shifts the vendor's own training-data risk onto us.
...
Vendor retains its models, algorithms, weights, and pre-existing technology; the customer retains its data, prompts, and Outputs. No customer IP is used to train models serving others. Feedback license narrowed to de-identified, aggregated use.
...
1. Vendor keeps its models, weights, and pre-existing technology; the customer keeps its data, prompts, and Outputs, with any feedback license limited to de-identified, aggregated use.
2. Feedback license permitted for de-identified, aggregated data only, with an express bar on using it to train customer-specific or competitor-serving models.
3. Vendor may use aggregated usage metrics for benchmarking, but not customer content, prompts, or Outputs.

**Walk-away floor**

Vendor asserts rights in the customer's data, prompts, or Outputs, or takes a feedback license that reaches model training.

**Escalation triggers**

- Vendor asserts rights in the customer's data, prompts, or Outputs
- Broad feedback / usage-data license that reaches model training

**Rationale**

This clause draws the line between the vendor's models and our data, prompts, and Outputs, so a broad feedback or usage-data license here quietly reopens the model-training risk the no-training default closes.
...
Vendor may use Customer inputs, outputs, and data ONLY to provide the service, and NOT to train, fine-tune, or improve any model that serves other customers, absent the customer's express written opt-in. Training is a one-way door -- a trained model cannot be untrained -- so any training license is effectively perpetual, irrevocable, and worldwide. Vendor deletes / does not retain training copies of customer data.
...
1. No training, fine-tuning, or model improvement on customer inputs, outputs, or data absent express written opt-in, with deletion of any training copies on termination.
2. Vendor may use only de-identified, aggregated data to improve the service, never in a customer-specific model, with a right to opt out of even that use.
3. Vendor may use operational data solely for security and abuse-prevention for a limited, disclosed retention period, and not to train or improve any model.

**Walk-away floor**

Vendor trains, fine-tunes, or improves models on our identifiable inputs, outputs, or data without our express written opt-in, or refuses to delete training copies.

**Escalation triggers**

- Vendor may train, fine-tune, or improve models on customer data or Outputs without opt-in
- License to 'improve the services' that reaches past service delivery into model training
- De-identification / aggregation used to circumvent the no-training rule
- No deletion / no-retention commitment for training copies

**Rationale**

Training on our data is irreversible and effectively perpetual, so a weak no-training default bakes our confidential inputs into a model that serves competitors with no way to claw it back.
...
As between the parties, the customer owns the Outputs generated from its use, and the vendor 'hereby assigns' any right it may have in those Outputs and will not reuse customer-specific Outputs for other customers. Inputs / prompts are treated as Customer Data. Because purely AI-generated content may not be copyrightable (US human-authorship requirement), this ownership grant is contractual as-between-the-parties and is paired with an output-infringement indemnity rather than relying on copyright.
...
1. Vendor assigns its rights in customer-specific Outputs and grants a perpetual, royalty-free, worldwide license to use them for any purpose, with no reuse of customer-specific Outputs.
2. Vendor grants the customer a perpetual, royalty-free license to the Outputs and agrees not to reuse customer-specific Outputs, even if formal assignment is declined.
3. Customer receives a perpetual, non-exclusive license to its Outputs, and the vendor may reuse only de-identified, aggregated Outputs that are not customer-specific.

**Walk-away floor**

Vendor claims ownership of customer Outputs, or reserves a right to reuse customer-specific Outputs for other customers.

**Escalation triggers**

- Vendor claims ownership of, or a license to reuse, customer Outputs
- Vendor reuses customer-specific Outputs for other customers
- No assignment of the vendor's rights in Outputs to the customer
- No output-infringement indemnity to backstop non-copyrightable Outputs

**Rationale**

Output ownership is the whole point of adopting the AI feature, so losing it, or letting the vendor reuse our Outputs for others, forfeits the value we are paying for.
